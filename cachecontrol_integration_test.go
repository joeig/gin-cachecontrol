...
func TestNewWithOptions(t *testing.T) {
	tests := []struct {
		name    string
		options []cachecontrol.ConfigOption
		want    string
	}{
		{
			name: "StandardCase",
			options: []cachecontrol.ConfigOption{
				...
				cachecontrol.WithNoCache(true),
				cachecontrol.WithNoStore(true),
				cachecontrol.WithMaxAge(cachecontrol.Duration(0)),
			},
			want: "must-revalidate, no-cache, no-store, max-age=0",
		},
		{
		...
		},
		{
			name: "RoundDuration",
			options: []cachecontrol.ConfigOption{
				cachecontrol.WithMaxAge(cachecontrol.Duration(1500 * time.Millisecond)),
				cachecontrol.WithSMaxAge(cachecontrol.Duration(1500 * time.Millisecond)),
				cachecontrol.WithStaleWhileRevalidate(cachecontrol.Duration(1500 * time.Millisecond)),
				cachecontrol.WithStaleIfError(cachecontrol.Duration(1500 * time.Millisecond)),
			},
			want: "max-age=2, s-maxage=2, stale-while-revalidate=2, stale-if-error=2",
		},
		{
			...
			options: []cachecontrol.ConfigOption{
				...
				cachecontrol.WithNoCache(true),
				cachecontrol.WithNoStore(true),
				cachecontrol.WithNoTransform(true),
				cachecontrol.WithPublic(true),
				cachecontrol.WithPrivate(true),
				cachecontrol.WithProxyRevalidate(true),
				cachecontrol.WithMaxAge(cachecontrol.Duration(1 * time.Second)),
				cachecontrol.WithSMaxAge(cachecontrol.Duration(2 * time.Second)),
				cachecontrol.WithImmutable(true),
				cachecontrol.WithStaleWhileRevalidate(cachecontrol.Duration(3 * time.Second)),
				cachecontrol.WithStaleIfError(cachecontrol.Duration(4 * time.Second)),
			},
			want: "must-revalidate, no-cache, no-store, no-transform, public, private, proxy-revalidate, max-age=1, s-maxage=2, immutable, stale-while-revalidate=3, stale-if-error=4",
		},
		{
			name: "NilOptionIgnored",
			options: []cachecontrol.ConfigOption{
				nil,
				cachecontrol.WithPublic(true),
			},
		...
		},
		{
			...
			options: []cachecontrol.ConfigOption{
				cachecontrol.WithPublic(true),
				cachecontrol.WithPublic(false),
				cachecontrol.WithPrivate(false),
				cachecontrol.WithPrivate(true),
			},
			want: "private",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			router := gin.New()
			router.Use(cachecontrol.NewWithOptions(tt.options...))
			router.Handle(testMethod, testPath, func(ginCtx *gin.Context) {
				ginCtx.Data(http.StatusOK, "", []byte{})
			})
